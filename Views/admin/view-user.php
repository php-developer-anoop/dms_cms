...
var user_detail = (data, type, row, meta) => {
 ...
 // let user_phone = row.user_phone != null ? row.user_phone : "";
  if (type === 'display') {
    ...
    data += '<span class="fotr_10"><b>Email : </b>' + user_email + '</span>';
...
function action_render(data, type, row, meta) {
  ...
  if (type === 'display') {
    ...
    output += '<a class="btn btn-sm btn-danger text-white" onclick="' + onclick + '" title="Delete User"><i class="tf-icons bx bx-trash"></i></a> ';
...
function status_render(data, type, row, meta) {
  if (type === 'display') {
    ...
    return `
    <div class="d-flex gap-3 checked-color">
